...
class Imprimir extends React.Component {
  ...
  CalcularImporte(listado) { // esta wea crea un arreglo con los importes totales por cada concepto 
                             // tienes q cambiar la parte q recibe un string por arreglo... es tu wea :vv
...
*/
   ...
   console.log("ESTE SI ES EL IMPORTE WE XD");
    console.log(importe[0]);
...
doc.text(f.getHours()+":"+f.getMinutes()+":"+f.getSeconds(), 540, 30);
    ...
    doc.setFontSize(11);
    doc.text("UNIVERSIDAD NACIONAL MAYOR DE SAN MARCOS", 175, 110);
    
    doc.setFont("helvetica");
    doc.setFontType("normal");
    doc.setFontSize(8);
    doc.text("Universidad del Peru, Decana de America", 245, 120);
    
    doc.setFont("helvetica");
    doc.setFontType("bold");
    doc.setFontSize(9);
    doc.text("SISTEMA DE PAGOS", 285, 140);  

    doc.setDrawColor(0, 0, 0);
    doc.setLineWidth(0.5);
    doc.line(175, 175, 450, 175);
    
    // doc.setDrawColor(0, 0, 0);
    // doc.setLineWidth(0.5);
    // doc.line(190, 175, 190, 190);

    doc.setFont("helvetica");
    doc.setFontType("bold");
    doc.setFontSize(11);
    doc.text("Datos del Estudiante", 192, 168); 
    ...
    doc.setFontType("bold");
    doc.setFontSize(9);
    doc.text("Facultad ", 192, 228); 

    doc.setFont("helvetica");
    doc.setFontType("bold");
    doc.setFontSize(9);
    doc.text("Codigo", 192, 188); 

    doc.setFont("helvetica");
    doc.setFontType("bold");
    doc.setFontSize(9);
    doc.text("Nombres y Apellidos", 192, 208); 

    doc.setFont("helvetica");
    doc.setFontType("normal");
    doc.setFontSize(9);
    doc.text(listafinal[0][0].nombre, 322, 228); 

    doc.setFont("helvetica");
    doc.setFontType("normal");
    doc.setFontSize(9);
    doc.text(nombres, 322, 188); 

    doc.setFont("helvetica");
    doc.setFontType("normal");
    doc.setFontSize(9);
    doc.text(listafinal[0][0].apeNom,322,208);
//       doc.text(importe.toString(), 322, 248);  // aqui esta le wea de los importes!!!!! XD
    ...
    var columns2 = ["Fecha","Documento","Numero","Importe","Concepto"];
    var marginT = 270;
    ...
    for(let i=0;i<listafinal.length;i++){
      ...
      doc.text("RESUMEN POR CONCEPTO DE PAGO "+conceptitos,192,marginT-10);
